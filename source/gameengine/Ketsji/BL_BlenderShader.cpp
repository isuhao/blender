...
RAS_InstancingBuffer::Usage BL_BlenderShader::GetInstancingAttribs() const
{
	GPUBuiltin builtins = GPU_get_material_builtins(m_gpuMat);

	RAS_InstancingBuffer::Usage usage = (RAS_InstancingBuffer::Usage)(RAS_InstancingBuffer::USE_MATRIX | RAS_InstancingBuffer::USE_POSITION);
	if (builtins & GPU_INSTANCING_COLOR_ATTRIB) {
		usage = (RAS_InstancingBuffer::Usage)(usage | RAS_InstancingBuffer::USE_COLOR);
	}
...
{
	GPU_material_bind_instancing_attrib(m_gpuMat, (void *)buffer->GetMatrixOffset(), (void *)buffer->GetPositionOffset(), (void *)buffer->GetColorOffset());
}
